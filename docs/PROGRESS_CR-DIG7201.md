...
| P2 | 会议主机真机联调（8 路话筒区分） | ✅ done | 2026-08-03 | | 正确 7 系列主机到货；协议实测纠偏(324B/大端BE)；修复后转写通顺→NPU纪要 |
...
- **P2 真机联调 done @2026-08-03（主 Claude，正确 7 系列主机）  commit: 见本次提交**
  ...
  **协议实测两处纠偏（原厂 doc 是工程草稿、注释里自己都打问号，抓真实流量为准）**：
  ① **包长 324B**（4 头 + 320B PCM = **160 sample**），非 doc 的 644B/320 sample → receiver 改**弹性包长**（4 头 + 偶数字节 PCM，324/644 都收，防型号变更）；300 包/秒 × 160 = 48kHz 自洽。
  ② **PCM 大端 BE**（包头 ID 也大端，全协议一致），非小端 → 原 `<i2` 把安静波形（大端小负 `ff f1`=-15）解成满量程噪声（`0xf1ff`=-3585），听感嗡嗡 + 转写幻听碎片。**一度误判"工频干扰/接地/削顶"，被用户一句"主机模拟监听清晰"点破 = 网络解码的锅**；改 `>i2` 后转写通顺。
  **端到端验收（3588 生产 meeting_asr + `audio.source: net_multicast`，`--force`/SIGKILL 保 funasr）**：切换后本地麦 audio_processor 停、net_audio_capture 起（8 路 join、坏包 0）、发言人分离准确（话筒N）、MQTT `command_punctuated` 发布通（web 收到 13 条）；大端后长句完整通顺（"…加紧组织对薛建平的预审，把郑文胜押回越山深挖余罪"）；NPU 1.7B **30s** 出结构化纪要，title/summary/points/keywords 齐、连贯。
  **遗留**：① 组播源电平偏低（大端峰值 ±400 = 满量程 1%），FunASR 靠 CMVN 归一化仍转对，是否加增益/降门限待真实会议观察；② **line-in 路放弃**（3588 无音频输入口，用户曾误插耳机输出口 RMS=7），USB 音频作模拟备选；③ 生产 config 已改 `audio.source: net_multicast` + receiver.py 已部署（备份 `.bak-cr7201-20260803`），config 是 gitignore → 入部署 SOP；④ 静音门"持续声源（听书连读无停顿）不结算 final"退化场景待兜底（门开超时强制结算）。
