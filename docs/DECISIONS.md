...
- ~~纪要机原音导出（net_multicast 路径）立项~~ → **用户 8/20 拍板必须补上，当日已实现**：`modules/net_audio_capture/recorder.py`（gate 后有声混音落盘 data/recordings/ + :5052 导出，静音不写盘、启动清理留 5 个会话、>4G 切片下载）。config `record_session` 默认开。待真机组播会议验证一场。
