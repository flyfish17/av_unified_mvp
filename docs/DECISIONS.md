...
### 2026-08-20
9. **出货版布局弹窗按 profile 灰置（用户 8/20 目检 .6 后拍板记档）**：meeting_asr 目检通过（单卡+nav 隐藏+话筒分段正常），但两处出货前要修：① 布局弹窗里被 profile 隐藏的模块仍可勾选重开——总部内用可以，**出货版应灰色不可点**（profile 隐藏 ≠ 用户偏好，不给客户开回来的入口）；② "客户视图"按钮在 meeting_asr 下无意义且点击后头部样式乱版（该功能是 full demo 的演示脸切换）——meeting_asr 下应隐藏该按钮。两处都是前端小改，凑下次部署窗口一起上。
...
- **纪要机原音导出（net_multicast 路径）立项**（2026-08-20 用户实测踩空暴露）：现"导出原音" :5052 服务在 audio_processor（本地麦路径），meeting_asr+组播形态下不起 → 按钮从来不可用（前端已止血：探活不通给提示）。补齐 = net_audio_capture 8 路 PCM 混音增量落盘（3h≈350MB，遵守"别全放内存"约束）+ :5052 嵌入式 HTTP 复用 audio_processor 模式。**量 0.5–1 人天 + 真机组播验证**。7 系列客户验收前是否要——待拍板。
