...
| **Paraformer-streaming** | MIT (FunASR) | 同 FunASR | funasr/paraformer-zh-streaming | ✅ 真 partial + ITN/标点 | 600ms chunk | ⚠️ **订正 5/19**：sherpa-onnx rk3588-*-paraformer-zh 全部是 N-seconds offline 固定窗口，**RK3588 NPU 无 paraformer streaming 资产**（5/19 路径 β 阶段 0 spike 实测确认，见 `spike-paraformer-streaming-3588-20260519.md`）。CPU streaming 可跑但 NPU 0%+ 无 ITN。RK3588 streaming 中文 ASR 只有 zipformer-bilingual（无 ITN） | 中（CPU only，NPU 需自转 RKNN，工时 >1w） |
