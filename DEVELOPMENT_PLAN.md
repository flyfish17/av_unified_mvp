...
| # | 任务 | 前置 / 边界 | 量 | 状态 |
|---|---|---|---|---|
| ① | **246 HA 上总线** | ✅ 8/22：用户开 62 broker 网段监听 + HA 加 MQTT 集成 + Statestream；token 存 62 config（示例文件只留空模板）；`web/server.py` `/api/ha/states` `/api/ha/call`（白名单 6 个服务）中转，Node-RED 不持 token；面板新增"家居 · Aqara"区动态渲染 8 实体；**闭环验证**：面板开关 → HA → Statestream → 面板 3s 内同步 | 1.5h | ✅ |
...
| ④ | **OBS 字幕路**：`/bigscreen?overlay=1&lines=N` 透明底/无顶栏/最近 N 段贴底（62 已部署，截图 alpha=0 验证）；OBS 端配置随演示环境 P0 上屏时做 | 页面侧 ✅ 8/22 | 1 天 | 🟡 页面完成，OBS 待上屏 |
| ⑤ | **湖森字幕接口** | 8/22 核实：主机已迁 **.64**（62 config 原 .253 已改）；当前固件 web bundle **无字幕页面**（i18n 有 `subtitle_*` 文案、零组件引用、无 API 调用），5/14 索引的 `/api/wall/subtitle` 不在此版前端。剩余路径：hscmd TCP 字幕指令（PDF）或问湖森 | 1 天 | ⛔ REST 路不通，转 hscmd/厂家 |
| ⑥ | **Light-ASD POC** | **算力先验 8/22（62 CPU 4 线程，纯前向）：1 脸 25fps ≈ 实时占比 88%，3 脸 242–360%** → 3588 CPU 只够单脸；多脸要 NPU（视觉编码 Conv3d 在 RKNN 支持存疑，需改写 2D+1D）或 10fps 降采样（精度未知）或外放。**POC 暂停，等机位 + 算力路线拍板** | 2–3 天 | ⏸ 先验不过线 |
...
- **① 完成（用户做 62 监听/HA 集成/Statestream/token，终端接线）**：HA 2024.9.0，156 实体（Aqara 面板开关×2、温湿度、人体×2、M1S 灯、窗帘×3 等，部分 unavailable 因正在更新）。Statestream `homeassistant/<domain>/<obj>/<attr>` 已在 62 总线。`web/server.py` 加 HA 中转（token 只在板上 config；`/api/ha/call` 只放行 turn_on/off/toggle/open/close/stop_cover）。Node-RED：mqtt-in `homeassistant/+/+/+` 聚 `ha_map` + 启动/5min 全量种子（`/api/ha/states`，Statestream 只发变化）+ 面板 `ha_call` → http → `/api/ha/call`；原 ASCII 路由节点对 `ha_call` 消息直接丢弃（防空 payload 发 .20）。两个坑：Dashboard 2.0 新页面 `msg` 初值不触发 watch → `immediate:true`；只回放最后一条 msg → 四个 function 改为三表（online/state/ha）合发。Playwright：家居区 8 行，M1S 灯 toggle 闭环 on→off 各 3s 内面板同步。**token 误写进 Mac 示例文件已清（未提交过）**。
...
- **⑥ Light-ASD 算力先验**：62 到 GitHub 不通，Mac 走代理浅克隆后拷板。权重 `pretrain_AVA_CVPR.model` 1.02M 参数加载（strict=False，4 个多余 key 为 loss 层）。输入：视觉 (B,T,112,112) 25fps 灰度、音频 (B,4T,13) MFCC 100fps。**62 CPU 4 线程纯前向**：1 脸 1s 窗 878ms（88% 实时）、2s 窗 1560ms；3 脸 1s 窗 2420ms（242%）、2s 窗 7193ms（360%）；吞吐 ≈ 30 脸帧/s。结论：会议室 ≥2 人时 CPU 实时不成立；且这还没算人脸检测与 MFCC。按 DECISIONS"先验算力"铁律，POC 暂停；若要推进，路线三选一：NPU（需把视觉编码 Conv3d 改 2D+1D 再转 RKNN，GRU 留 CPU）/ 10fps 降采样验精度 / 外放 Mac。
- **④ 字幕叠加页**：`/bigscreen?overlay=1&lines=N`（`html.overlay` 透明底、隐顶栏/待机层、`#flow` 贴底只留 N 段、半透明底框+文字描影）。62 部署后 Playwright `omit_background` 截图角点 alpha=0、段数=2、贴底 692/720。OBS 浏览器源直接用此 URL 叠在视频上。
- **⑤ 湖森字幕**：`.253` 无路由→ 湖森 HDC910 主机实际在 **.64**（:80/:6000 通），62 `husion.host` 改 .64（此前 husion_distributed 一直 poll 失败）。拉取 app.js（6.7MB，需 Referer）静态分析：`subtitle` 88 处全是 3 语言包文案 + 图标，**无组件引用、无 `/api/wall/subtitle` 调用**——当前固件 web 不提供字幕功能。REST 路判死；hscmd TCP 字幕指令需厂家 PDF 样本。
